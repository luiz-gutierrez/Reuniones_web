// src/pages/jefe_depto/Tareas.jsx
...
export default function JefeDeptoTareas() {
  ...
  // ========== ACTUALIZAR ESTADO - JEFE DEPTO ==========
  const actualizarEstado = async (tareaId, nuevoEstatus) => {
    // ✅ Estados válidos para JefeDepto: Iniciar, Proceso, Prerevision
    const estatusValidos = ['Iniciar', 'Proceso', 'Prerevision'];
    if (!estatusValidos.includes(nuevoEstatus)) {
      alert('❌ Estado no válido para Jefe de Departamento');
      return;
    }
    ...
    // ✅ Validar que no se pueda pasar a Prerevision si ya está en Prerevision o superior
    if (nuevoEstatus === 'Prerevision' && ['Prerevision', 'Revision', 'Finalizado'].includes(estadoActual)) {
      alert(`❌ No puedes enviar a Prerevision una tarea que ya está en "${estadoActual}"`);
      return;
    }

    // ✅ Mensaje de confirmación personalizado
    let mensajeConfirmacion = '';
    if (nuevoEstatus === 'Prerevision') {
      mensajeConfirmacion = '¿Estás seguro de enviar esta tarea a Pre-revisión (Gerente)?';
  ...
  const tareasFiltradas = tareas.filter(tarea => {
    ...
    if (filtroEstatus === 'pendientes') {
      estatusMatch = ['Iniciar', 'Proceso'].includes(tarea.tar_estatus);
  ...
  const getTitulo = () => {
    switch(filtroEstatus) {
      ...
      case 'prerevision': return 'Tareas en Pre-revisión';
      case 'finalizadas': return 'Tareas Finalizadas';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1 flex items-center gap-2">
          ...
          {user?.nombre} {user?.apellido} - Jefe de Departamento
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 font-medium flex items-center gap-1">
                <FaSearch className="text-purple-500" /> En revision con gerente
              </p>
              <p className="text-3xl font-bold text-purple-600 mt-1">{estadisticas.prerevision}</p>
            </div>
            <div className="w-12 h-12 bg-purple-100 rounded-xl flex items-center justify-center text-2xl">
              <FaSearch className="text-purple-600 text-3xl" />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value="prerevision">📤 Revicion con gerente</option>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-12 text-center border border-gray-100">
          ...
          <h3 className="text-xl font-semibold text-gray-700 mb-2">
            {filtroEstatus === 'pendientes' ? '¡No hay tareas pendientes!' : 
             filtroEstatus === 'prerevision' ? 'No hay tareas en Pre-revisión' : 
                ...
                <div className="mt-4 pt-3 border-t border-gray-100">
                  {['Iniciar', 'Proceso'].includes(tarea.tar_estatus) ? (
                    <div className="relative">
                      ...
                      >
                        {/* ✅ Siempre disponibles: Iniciar, Proceso, Prerevision */}
                        <option value="Iniciar">🚀 Iniciar</option>
                        <option value="Proceso">🔄 En Proceso</option>
                        <option value="Prerevision">📤 Enviar a Gerente a revisión</option>
                      </select>
                    ...
                    <div className="flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium bg-purple-50 text-purple-700">
                      <FaClock className="text-purple-500" />
                      En espera de aprobación del Gerente
                    </div>
                    ...
                    <div className="flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium bg-indigo-50 text-indigo-700">
                      ...
                      En revisión final por Secretaria
                    </div>
                  ) : (
